...
//
// Follow up: 
//    Could you implement a solution using only O(1) extra space complexityand O(n) runtime complexity ?
// Example 1:
//    Input : nums = [3, 0, 1]
//    Output : 2
...
int missingNumber(vector<int>&& nums) { // 1,0
    for (int i = 0; i < nums.size(); i++) {
        if (nums[i] == 0)
            continue;

        while (nums[i] != 0 && nums[i] - 1 != i) { // 0 != 1
            int tmp = nums[nums[i] - 1]; // 0
